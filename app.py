...
def update_m3u_background():
    ...
    try:
        ...
        for ch in channels:
            ...
            # 2. ਸੈਕੰਡਰੀ (ਬੈਕਅੱਪ) ਚੈਨਲ ਐਂਟਰੀ ਲਿਖੋ (ਜੇ ਉਪਲਬਧ ਹੋਵੇ)
            ...
            if sec_block:
                # ਉਹੀ ਨਾਂ, LCN ਅਤੇ ਪ੍ਰਾਇਮਰੀ ਲੋਗੋ ਵਰਤ ਕੇ ਬੈਕਅੱਪ ਚੈਨਲ ਬਣਾਓ
                m3u += f'#EXTINF:-1 tvg-id="{ch_id}_sec" ch-number="{ch_no}" group-title="{group}" group-logo="{group_logo}" tvg-logo="{logo}",{formatted_name}\n'
                
                # ਸੈਕੰਡਰੀ ਬਲਾਕ ਦੀਆਂ ਆਪਣੀਆਂ ਪ੍ਰੋਪਰਟੀਜ਼ ਅਤੇ ਲਿੰਕ ਸ਼ਾਮਲ ਕਰੋ
                m3u += f'{sec_block}\n\n'
        ...
        cached_m3u = m3u
        cached_epg = epg
        print("Playlist with Dual Channels (Same Name, LCN, Logo) updated successfully.")
...
@app.route('/')
def home():
    return "JioTV Dual Channel Playlist Server is Running!"
